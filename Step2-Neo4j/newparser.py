...
filedata = """TRUNCATE PUBLISHER_05 CASCADE;\nINSERT INTO PUBLISHER_05(publisherid,publisher_name,Country) VALUES"""
...
with open(destination_path, 'w', encoding="utf-8") as file:
    ...
    for i, j in enumerate(corrected_publisher_dict):
        ...
        file.write(f"""\n('{corrected_publisher_dict[j]}', '{j}','{country}')""")
    file.write('\n;')
file.close()
...
# CSV with nationalities for authors
nation = pd.read_csv('CH_Nationality_List_20171130_v1.csv')
...
with open(destination_path, 'a', encoding="utf-8") as file:
    file.write(filedata)
    for index, batch in enumerate(batch_list[1:]):
        file.write("""INSERT INTO AUTHOR_05(authorid,author_name,gender,nationality) VALUES\n""")
        ...
        for i in authors_batch[:-1]:
            ...
            name_string = i['name']
            number = np.random.randint(0, len(nation)-1)
            nationality = nation.at[number, 'Nationality']
            if number < (len(nation)/2):
                gender = 'F'
            else:
                gender = 'M'
            file.write(f"""('{id_string}','{name_string}','{gender}','{nationality}'),\n""")
        number = np.random.randint(0, len(nation)-1)
        nationality = nation.at[number, 'Nationality']
        if number < (len(nation) / 2):
            gender = 'F'
...
users = pd.read_excel(current_path + "\\Comic_Book_Users_Orders2.xlsx",  sheet_name='Users')
orders = pd.read_excel(current_path + "\\Comic_Book_Users_Orders2.xlsx",  sheet_name='Orders')
